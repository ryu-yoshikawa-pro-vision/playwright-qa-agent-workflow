...
テスト生成後は、次のファイルも現行情報として扱います。

```text
specs/<feature>.coverage.md
```

これは、現在実装済みのテスト、設計ケースとの対応、明示的に確認していない内容、未解決事項、assertion 方針を一か所で読むための coverage 台帳です。`artifacts/<feature>/runs/<run-id>/04_generator/test-mapping.md` は履歴であり、現行 coverage の正本ではありません。
