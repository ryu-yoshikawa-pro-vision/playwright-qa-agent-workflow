...
対象アプリの URL、認証、使用するロール、データ操作ルールを中心に、必要に応じて対象プロジェクト情報、テスト実行コマンド、生成方針などをまとめます。
...
`Target repository`、`Branch`、`Local path` は、Playwright Test の生成、対象プロジェクト側でのテスト実行、失敗テスト修復を行う場合に記入します。

Playwright CLI でブラウザ探索だけを行う場合は、`N/A` または `Not used for browser exploration` と書いて構いません。
...
`Local path` は、AI エージェントが対象プロジェクトを読める場所です。探索だけの場合は `N/A` で構いません。
...
探索だけを行う場合は、このセクションを `N/A` または `Not used for browser exploration` として構いません。
...
探索だけの場合は、対象リポジトリ、ローカルパス、テスト実行コマンドを `N/A` として構いません。
...
- 生成・実行・修復を行う場合、対象プロジェクトのテスト実行コマンドが明確である
- データ操作ルールが明確である
- generator を使う場合、生成先と locator / assertion 方針が明確である
