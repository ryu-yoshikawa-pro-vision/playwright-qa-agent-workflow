...
このガイドは、`agents-playwright` を使って Web アプリケーションの探索、テスト計画、テスト設計、Playwright Test 生成、失敗テストの診断を進める人のための説明です。

Playwright や AI エージェントを初めて使う人でも、上から順に読めば作業を始められるように、専門用語をできるだけ作業手順に置き換えて説明します。

## 最初に理解すること

`agents-playwright` は、テスト対象の Web アプリケーションそのものではありません。
...
| 順番 | ファイル | 目的 |
| --- | --- | --- |
| 1 | [`01-before-you-start.md`](01-before-you-start.md) | 作業前に必要な準備を確認する |
| 2 | [`02-target-project-profile.md`](02-target-project-profile.md) | 対象アプリの URL、認証、データ方針、テスト実行方法を整理する |
| 3 | [`03-service-exploration.md`](03-service-exploration.md) | 対象アプリ全体を探索する |
| 4 | [`04-feature-workflow.md`](04-feature-workflow.md) | 1機能をテスト計画、設計、生成まで進める |
| 5 | [`05-artifact-reading-guide.md`](05-artifact-reading-guide.md) | どの成果物を見れば現在状態が分かるか確認する |
| 6 | [`06-troubleshooting.md`](06-troubleshooting.md) | 困ったときの確認方法を調べる |
| 7 | [`glossary.md`](glossary.md) | 用語の意味を確認する |

## 最短で作業を始める場合

すでに対象アプリの URL、アカウント、テスト実行コマンド、データ操作ルールが分かっている場合は、次の流れで開始します。
...
パスワード、Cookie、アクセストークン、リフレッシュトークン、API キー、保存済みブラウザ状態は、成果物や Git に残してはいけません。
